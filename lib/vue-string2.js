...
import { parseFuncName, n } from './util'
...
class VueString {
  ...
  transScript() {
    let script_arr = [
      `<script>`,
      `export default {`,
      `\fname: "${this.json.name}",`,
      `\fdata() { ${getData(2)} },`,
      `\fmethods() { ${getMethod(2)} },`,
      `\fcreated() { \f\fthis.init() },`,
      `},`,
      `</script>`,
    ]
    return `${script_arr.join(`\n`)}\n`
  }
...
const template = json => {
  ...
  string += `
    <a-table :columns="table_fields" :data-source="table_lists" :loading="loading" bordered :pagination="false">${parseSlots(json)}
    </a-table>`;
  if ((json.action && json.action.includes('add-modal'))
    || (json.tableAction && json.tableAction.includes('edit-modal'))
  ) {
...
module.exports = json => {
  return {
    name: 'Table.vue',
    code: `${template(json)}${script(json)}${style(json)}`
  }
}
